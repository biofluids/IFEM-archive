...
subroutine block(xloc, dloc, doloc, p, q, hk, ien, f_fluids)
  ...
  integer ien(nen,ne),rng(neface,ne),rngface(neface,ne)
  real* 8 xloc(nsd,nn)
  ...
  integer inl, ie, isd, iq, node,jsd,ieface,irng,inface
...
! add in pressure driven input
     ...
     do ieface=1,neface
        ...
        if ((irng == 5)) then ! .and. (irng <=264)) then
            ! write(*,*) irng
           ! call getnormal(normal,ie,ieface,ien,xloc)
            !write(*,*) normal(1)
            !write(*,*) normal(2)
            !write(*,*) normal(3)
