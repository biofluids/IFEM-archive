...
subroutine update_x_inter(x,x_inter,x_inter_ini,vel_inter,vel_fluid,vel_fluid_old,vol_nn,dt,I_solid)
  ...
  real(8) vol_nn(nn),dt,I_solid(nn)
  ...
  x_inter_temp(:,:)=0.0
  x_inter_ini_temp(:,:)=0.0
  vel_inter_temp(:,:)=0.0

  do loc_index=1,nn_inter_loc
     i=myid+1+(loc_index-1)*ncpus
     xloc_ini(:)=x_inter(:,i)
     call get_inter_vel(x,xloc_ini,vel_fluid_old,vel_loc,vol_nn,I_solid)
     ...
     call get_inter_vel(x,xloc_temp,0.5*(vel_fluid(1:nsd,:)+vel_fluid_old(1:nsd,:)),vel_loc,vol_nn,I_solid)
     R_K(:)=vel_loc(:)*dt
     ...
     call get_inter_vel(x,xloc_temp,vel_fluid,vel_loc,vol_nn,I_solid)
     R_K(:)=vel_loc(:)*dt
...
call mpi_allreduce(x_inter_ini_temp(1,1),x_inter_ini(1,1),nsd*maxmatrix,mpi_double_precision,mpi_sum,&
			mpi_comm_world,ierror)
