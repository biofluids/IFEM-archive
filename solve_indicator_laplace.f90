  ...
  subroutine solve_indicator_laplace(x,x_inter,x_center,I_fluid,I_solid,I_fluid_center,ien,center_mapping,&
  ...
  real(8) I_fluid(nn),I_fluid_center(nn_center),I_solid(nn)
  ...
  do ie=1,ne
     ...
     do inl=1,nen
	node=ien(inl,ie)
	if(abs(I_fluid(node)-0.5).gt.0.3) then
	  node_id(node)=0
	  flag=flag+1
