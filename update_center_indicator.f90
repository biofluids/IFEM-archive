...
subroutine update_center_indicator(x,x_inter,x_center,vel_fluid,vol_nn,dt,I_fluid_center,corr_Ip)
  ...
  real(8) delta_I(ne_den_domain),delta_I_temp(ne_den_domain),hg(ne)
  ...
  do loc_index=1,nn_loc
     ...
     xloc(:)=x_center(:,ie)
     if(nsd==2) then
	call get_indicator_derivative_2D_1st(xloc,x_inter,x_center,hg, &
			I_fluid_center,corr_Ip,II,dI,ddI,norm_p,curv_p)
     else if(nsd==3) then
	call get_indicator_derivative_3D_1st(xloc,x_inter,x_center,hg, &
			I_fluid_center,corr_Ip,II,dI,ddI,norm_p,curv_p)
     end if
