...
subroutine res_rotation_reverse(p,norm_node,nn_spbc,spbcnode)
...
if(nsd==2) then
  do i=1,nn_spbc
     ...
     tang(1)=-norm(2)
     tang(2)=norm(1)
     temp(1)=norm(1)*p(1,node)+tang(1)*p(2,node)
     temp(2)=norm(2)*p(1,node)+tang(2)*p(2,node)
     p(1,node)=temp(1)
     p(2,node)=temp(2)
  end do
...
elseif(nsd==3) then
  ...
  do i=1,nn_spbc
     ...
     temp(1)=norm(1)*p(1,node)+tang1(1)*p(2,node)+tang2(1)*p(3,node)
     temp(2)=norm(2)*p(1,node)+tang1(2)*p(2,node)+tang2(2)*p(3,node)
     temp(3)=norm(3)*p(1,node)+tang1(3)*p(2,node)+tang2(3)*p(3,node)
...
2000 continue
end subroutine res_rotation_reverse
