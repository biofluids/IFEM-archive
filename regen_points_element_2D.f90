...
subroutine regen_points_element_2D(xn,x_inter,x_center,hg,I_fluid_center,corr_Ip,&
				norm_e,nn_regen_proc,x_regen_proc,ie)

  use fluid_variables,only:nsd,nn,ne,nen,nnface
  use interface_variables
  use mpi_variables

!nnface=2 for 2D

  real(8) xn(nsd,nnface),x_inter(nsd,maxmatrix),x_center(nsd,nn_center),hg(ne)
  ...
  real(8) norm(nsd),tangx(nsd)

  integer i,j,k,icount,jcount,kcount,isd,ie
  real(8) temp,modmod

  real(8) xloc(nsd,nnface)
...
!  x_loc_can(:,:)=0.0
  tangx(:)=xn(:,2)-xn(:,1)
  modmod=sqrt(tangx(1)**2+tangx(2)**2)
  tangx(:)=tangx(:)/modmod
  ...
  temp=norm(1)*norm_e(1)+norm(2)*norm_e(2)
  if(temp.gt.0.0) distance_sign=1.0
  if(temp.le.0.0) distance_sign=-1.0


  xloc(1,:)=tangx(1)*(xn(1,:)-xn(1,1))+tangx(2)*(xn(2,:)-xn(2,1))
  xloc(2,:)=norm(1) *(xn(1,:)-xn(1,1))+norm(2) *(xn(2,:)-xn(2,1))
...
!     end do
     xlocan(1)=0.5*(xloc(1,1)+xloc(1,2))
     xlocan(2)=distance_sign*hg_sp/20.0  ! slightly lift the point

!     xlocan(:)=xlocan(:)+xn(:,1)
     x_tran(1)=tangx(1)*xlocan(1)+norm(1)*xlocan(2)
     x_tran(2)=tangx(2)*xlocan(1)+norm(2)*xlocan(2)
     xlocan(:)=x_tran(:)+xn(:,1)
xlocan_ini(:)=xlocan(:)
...
100 continue
end subroutine regen_points_element_2D
