...
subroutine generate_contactline_2D(x_inter_regen,nn_inter_regen,x,x_inter,x_center,hg,I_fluid_center, &
...
!  use allocate_variables
  ...
  real(8) interval,tangx(nsd),modmod,temp
  ...
  do loc_index=1,nn_loc
     ...
     call get_indicator_derivative_2D(xc,x_inter,x_center,hg,I_fluid_center,corr_Ip,II,dI,ddI,norm_c,curv_c)
     xlocan(:)=xc(:)
     icount=1
     x_cor_regen(:,:)=0.0
	...
	do while((nitt.le.5).and.(err_p.gt.1.0e-6))
	   temp=sqrt(deltaa(1)**2+deltaa(2)**2)
	   if(temp.gt.hg_sp) deltaa(:)=deltaa(:)/temp*hg_sp
	   xlocan(:)=xlocan(:)+deltaa(:)
	   call get_indicator_derivative_2D_1st(xlocan,x_inter,x_center,hg,I_fluid_center,corr_Ip,II,dI,ddI,norm_g,curv_g)
...
!          if((xlocan(1)-xc(1))*norm_e(1)+(xlocan(2)-xc(2))*norm_e(2).lt.0.5*hg_sp)x_vel(:)=xlocan(:)
	  if( (xlocan(1)-xc(1))*norm_e(1)+(xlocan(2)-xc(2))*norm_e(2).le.0.9*hg_sp) then
	      ...
	      vec_contact(:)=tangx(:) !tangential direction of wall, used to cal contact vel
              if(tangx(1)*norm_g(1)+tangx(2)*norm_g(2).lt.0)vec_contact(:)=-tangx(:)
	      xr(:)=xlocan(:)
	  end if
	 ...
	 if(flag_ca==0) then
	    ...
	    do jcount=IndexRef+1,NumRegen
	       ...
	       x_contact_proc(:,nn_contact_proc)=x_cor_regen(:,jcount)
            ...
            tangx(1)=-norm_e(2)
            tangx(2)=norm_e(1)
            modmod=tangx(1)*norm_r(1)+tangx(2)*norm_r(2)
            if(modmod.lt.0.0) tangx(:)=-tangx(:)

	    temp=(xr(1)-xc(1))*norm_e(1)+(xr(2)-xc(2))*norm_e(2)
	    x_fix(1)=0.0
	    x_fix(2)=0.0
	    x_wall(1)=0.0
	    x_wall(2)=-temp!0.5*max_hg
	    norm_fix(1)=norm_r(1)*tangx(1)+norm_r(2)*tangx(2)
	    norm_fix(2)=norm_r(1)*norm_e(1)+norm_r(2)*norm_e(2)
	    ...
	    do jcount=1,nn_2d
	       nn_contact_proc=nn_contact_proc+1
	       x_contact_proc(1,nn_contact_proc)=tangx(1)*x_2d(1,jcount)+norm_e(1)*x_2d(2,jcount)
	       x_contact_proc(2,nn_contact_proc)=tangx(2)*x_2d(1,jcount)+norm_e(2)*x_2d(2,jcount)
...
!if(myid==0) then
!write(222,*)'#Version 1.0' 
!write(222,*)'#EnSight Point Format '
!do i=1,nn_contact_proc
!   write(222,*)x_contact_proc(:,i),0.0
!end do
!end if
!
!close(222)
...
999 continue
end subroutine generate_contactline_2D
