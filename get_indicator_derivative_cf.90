...
subroutine get_indicator_derivative_cf(x,xp,x_center,hg,I_fluid_center,corr_Ip, &
				  II,dI,ddI,&
				  norm_p, curv_p)
...
!	  if(flag==1) then
	   ...
	   if(abs(dx).gt.3.0*hsp) index_center(j,inode)=0
!	  else if(flag==2) then
!	   dx=x(jsd)-xp(jsd,j)
...
!==========================================
  ...
  do j=1,nn_center_domain
   do inode=1,1
   if(index_center(j,inode)==1) then
...
!    end do
	...
	do jcount=1,nsd+1
	   ...
	   M(icount,jcount,1)=M(icount,jcount,1)+Mvec(icount,jcount,1)*Phi(1,j)*wp!/(hsp**nsd)*(hsg**nsd) !M

	   M(icount,jcount,2)=M(icount,jcount,2)+(Mvec(icount,jcount,2)*Phi(1,j)+ &
						  Mvec(icount,jcount,1)*Phi(2,j))*wp!/(hsp**nsd)*(hsg**nsd)

	   M(icount,jcount,3)=M(icount,jcount,3)+(Mvec(icount,jcount,3)*Phi(1,j)+ &
						  Mvec(icount,jcount,1)*Phi(3,j))*wp!/(hsp**nsd)*(hsg**nsd)

	   M(icount,jcount,4)=M(icount,jcount,4)+(Mvec(icount,jcount,4)*Phi(1,j)+ &
						  Mvec(icount,jcount,1)*Phi(4,j))*wp!/(hsp**nsd)*(hsg**nsd)

	   M(icount,jcount,5)=M(icount,jcount,5)+(Mvec(icount,jcount,5)*Phi(1,j)+ &
						  2*Mvec(icount,jcount,2)*Phi(2,j)+ &
						  Mvec(icount,jcount,1)*Phi(5,j))*wp!/(hsp**nsd)*(hsg**nsd)

	   M(icount,jcount,6)=M(icount,jcount,6)+(Mvec(icount,jcount,6)*Phi(1,j)+ &
						  2*Mvec(icount,jcount,3)*Phi(3,j)+ &
						  Mvec(icount,jcount,1)*Phi(6,j))*wp!/(hsp**nsd)*(hsg**nsd)

           M(icount,jcount,7)=M(icount,jcount,7)+(Mvec(icount,jcount,7)*Phi(1,j)+ &
                                                  2*Mvec(icount,jcount,4)*Phi(4,j)+ &
                                                  Mvec(icount,jcount,1)*Phi(7,j))*wp!/(hsp**nsd)*(hsg**nsd)

	   M(icount,jcount,8)=M(icount,jcount,8)+(Mvec(icount,jcount,8)*Phi(1,j)+ &
						  Mvec(icount,jcount,2)*Phi(3,j)+ &
						  Mvec(icount,jcount,3)*Phi(2,j)+ &
						  Mvec(icount,jcount,1)*Phi(8,j))*wp!/(hsp**nsd)*(hsg**nsd)

           M(icount,jcount,9)=M(icount,jcount,9)+(Mvec(icount,jcount,9)*Phi(1,j)+ &
                                                  Mvec(icount,jcount,2)*Phi(4,j)+ &
                                                  Mvec(icount,jcount,4)*Phi(2,j)+ &
                                                  Mvec(icount,jcount,1)*Phi(9,j))*wp!/(hsp**nsd)*(hsg**nsd)

           M(icount,jcount,10)=M(icount,jcount,10)+(Mvec(icount,jcount,10)*Phi(1,j)+ &
                                                  Mvec(icount,jcount,3)*Phi(4,j)+ &
                                                  Mvec(icount,jcount,4)*Phi(3,j)+ &
                                                  Mvec(icount,jcount,1)*Phi(10,j))*wp!/(hsp**nsd)*(hsg**nsd)
	end do
    end do
   end if  !!!!!!!
   end do ! end of inode
...
!stop
  ...
  do icount=1,nsd+1
     do jcount=1,nsd+1
        P(icount)=P(icount)-M(icount,jcount,7)*B(jcount,1)- &
                            2*M(icount,jcount,4)*B(jcount,4)
     end do
  end do
  Mtemp(1:nsd+1,1:nsd+1)=M(1:nsd+1,1:nsd+1,1)
  call DGESV(nsd+1,1,Mtemp,nsd+1,IP,P,nsd+1,INFO)
  ...
  do icount=1,nsd+1
     do jcount=1,nsd+1
	P(icount)=P(icount)-M(icount,jcount,8)*B(jcount,1)- &
  ...
  do icount=1,nsd+1
     do jcount=1,nsd+1
        P(icount)=P(icount)-M(icount,jcount,9)*B(jcount,1)- &
                            M(icount,jcount,2)*B(jcount,4)- &
                            M(icount,jcount,4)*B(jcount,2)
     end do
  end do
  Mtemp(1:nsd+1,1:nsd+1)=M(1:nsd+1,1:nsd+1,1)
  call DGESV(nsd+1,1,Mtemp,nsd+1,IP,P,nsd+1,INFO)
  ...
  do icount=1,nsd+1
     do jcount=1,nsd+1
        P(icount)=P(icount)-M(icount,jcount,10)*B(jcount,1)- &
                            M(icount,jcount,3)*B(jcount,4)- &
                            M(icount,jcount,4)*B(jcount,3)
     end do
  end do
  Mtemp(1:nsd+1,1:nsd+1)=M(1:nsd+1,1:nsd+1,1)
  call DGESV(nsd+1,1,Mtemp,nsd+1,IP,P,nsd+1,INFO)
...
!====construct vec vecx vecy vecxx vecyy vecxy==========
	...
	dtemp(4)=dtemp(4)+vec(icount,4)*B(icount,1)+vec(icount,1)*B(icount,4)

	dtemp(5)=dtemp(5)+vec(icount,5)*B(icount,1)+ &
			  2*vec(icount,2)*B(icount,2)+vec(icount,1)*B(icount,5)
	dtemp(6)=dtemp(6)+vec(icount,6)*B(icount,1)+ &
			  2*vec(icount,3)*B(icount,3)+vec(icount,1)*B(icount,6)
        dtemp(7)=dtemp(7)+vec(icount,7)*B(icount,1)+ &
                          2*vec(icount,4)*B(icount,4)+vec(icount,1)*B(icount,7)

	dtemp(8)=dtemp(8)+vec(icount,8)*B(icount,1)+vec(icount,2)*B(icount,3)+ &
			  vec(icount,3)*B(icount,2)+vec(icount,1)*B(icount,8)

        dtemp(9)=dtemp(9)+vec(icount,9)*B(icount,1)+vec(icount,2)*B(icount,4)+ &
                          vec(icount,4)*B(icount,2)+vec(icount,1)*B(icount,9)

        dtemp(10)=dtemp(10)+vec(icount,10)*B(icount,1)+vec(icount,3)*B(icount,4)+ &
                          vec(icount,4)*B(icount,3)+vec(icount,1)*B(icount,10)

     end do

     W0=dtemp(1)*Phi(1,j)/(hsp**nsd)*(hsg**nsd)
     W1(1)=(dtemp(2)*Phi(1,j)+dtemp(1)*Phi(2,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W1(2)=(dtemp(3)*Phi(1,j)+dtemp(1)*Phi(3,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W1(3)=(dtemp(4)*Phi(1,j)+dtemp(1)*Phi(4,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(1)=(dtemp(5)*Phi(1,j)+2*dtemp(2)*Phi(2,j)+dtemp(1)*Phi(5,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(2)=(dtemp(6)*Phi(1,j)+2*dtemp(3)*Phi(3,j)+dtemp(1)*Phi(6,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(3)=(dtemp(7)*Phi(1,j)+2*dtemp(4)*Phi(4,j)+dtemp(1)*Phi(7,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(4)=(dtemp(8)*Phi(1,j)+dtemp(2)*Phi(3,j)+dtemp(3)*Phi(2,j)+dtemp(1)*Phi(8,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(5)=(dtemp(9)*Phi(1,j)+dtemp(2)*Phi(4,j)+dtemp(4)*Phi(2,j)+dtemp(1)*Phi(9,j))*wp!/(hsp**nsd)*(hsg**nsd)
     W2(6)=(dtemp(10)*Phi(1,j)+dtemp(3)*Phi(4,j)+dtemp(4)*Phi(3,j)+dtemp(1)*Phi(10,j))*wp!/(hsp**nsd)*(hsg**nsd)
     ...
     ddI(4:6)=ddI(4:6)+W2(4:6)*I_fluid_center(center_domain(j))
   end if !!!!
  end do ! end of inode
  end do
...
!     end do
     ...
     W1(3)=(dtemp(4)*Phi_inter(1,j)+dtemp(1)*Phi_inter(4,j))!/(hsp**nsd)*(hsg**nsd) 
     W2(1)=(dtemp(5)*Phi_inter(1,j)+2*dtemp(2)*Phi_inter(2,j)+dtemp(1)*Phi_inter(5,j))!/(hsp**nsd)*(hsg**nsd)
     W2(2)=(dtemp(6)*Phi_inter(1,j)+2*dtemp(3)*Phi_inter(3,j)+dtemp(1)*Phi_inter(6,j))!/(hsp**nsd)*(hsg**nsd)
     W2(3)=(dtemp(7)*Phi_inter(1,j)+2*dtemp(4)*Phi_inter(4,j)+dtemp(1)*Phi_inter(7,j))!/(hsp**nsd)*(hsg**nsd)
     W2(4)=(dtemp(8)*Phi_inter(1,j)+dtemp(2)*Phi_inter(3,j)+dtemp(3)*Phi_inter(2,j)+dtemp(1)*Phi_inter(8,j))!/(hsp**nsd)*(hsg**nsd)
  ...
  temp=sqrt(dI(1)**2+dI(2)**2+dI(3)**2)
  norm_p(1:nsd)=-dI(1:nsd)/temp

     curv_A=ddI(1)/temp+dI(1)*(-0.5)/(temp**3)*(2*dI(1)*ddI(1)+2*dI(2)*ddI(4)+2*dI(3)*ddI(5))
...
123 continue
end subroutine get_indicator_derivative_cf
