...
subroutine gmres_ps(x,d,dold,w,bg,bg_p,dg,hg,ien,fext,id, &
	...
	real* 8 bg_p(ndf,nn) ! PS residual
!=========================================================
...
! MPI varibalbes
  ...
  real(8) space2(inner)
        ...
        real(8) end_time
!--------------------------------------
! To use PS and because Vm is defined on local
 real(8) tmp_vm(ndf*nn)
!---------------------------------------------
...
! dv(:)=0.0
			...
			do jcount=1,ndf
				kcount=(node-1)*ndf+jcount
                   		dv(kcount)=0.0d0
				tmp_vm(kcount)=Vm((icount-1)*ndf+jcount,j) ! form a full Vm(:,j)
			end do
                end do
...
!===========================
!                 call gmres_com(x,d,dold,w,Vm(:,j),bg_p,dv,hg,ien,fext,id)
        call gmres_com(x,d,dold,w,tmp_vm,bg_p,dv,hg,ien,fext,id, &
                   ne_local,ien_local,node_local,nn_local, &
                   global_com,nn_global_com,local_com,nn_local_com,send_address,ad_length)
		 ...
		 do icount=1, nn_local
		    node=node_local(icount)
		    do jcount=1,ndf
		    	dv((node-1)*ndf+jcount) = eps*dv((node-1)*ndf+jcount)
		    end do
...
!---------------------------------
!        call gmres_com(x,d,dold,w,x0,bg_p,dv,hg,ien,fext,id)
        call gmres_com(x,d,dold,w,x0,bg_p,dv,hg,ien,fext,id, &
                   ne_local,ien_local,node_local,nn_local, &
                   global_com,nn_global_com,local_com,nn_local_com,send_address,ad_length)
...
111     continue  ! end outer loop
	...
	dg(:)=0.0d0
        call gmres_com(x,d,dold,w,x0,bg_p,dg,hg,ien,fext,id, &
                   ne_local,ien_local,node_local,nn_local, &
                   global_com,nn_global_com,local_com,nn_local_com,send_address,ad_length)
