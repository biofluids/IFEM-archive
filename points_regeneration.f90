...
subroutine points_regeneration(x,x_inter,x_center,x_inter_regen,nn_inter_regen,I_fluid_center,corr_Ip,ien,intflag,&
  ...
  real(8) I_solid(nn),I_solid_ele(nen)
  ...
  do loc_index=1,nn_loc
	...
	x_fluid(1:nsd,inl)=x(1:nsd,node)
        I_solid_ele(inl)=I_solid(node)
     end do
    
     do icount=1,nn_sub**nsd
        II=0.0
        do inl=1,nen
           II=II+sh(inl,icount)*I_solid_ele(inl)
        end do
        if(II.gt.I_solid_inter)goto 333
	   ...
	   xlocan(1:nsd)=xlocan(1:nsd)+sh(inl,icount)*x_fluid(1:nsd,inl)
        ...
        if(nsd==2) then
           call get_indicator_derivative_2D_1st(xlocan,x_inter,x_center,&
                                I_fluid_center,corr_Ip,II,dI,ddI,norm_p,curv_p)
        else if(nsd==3) then
           call get_indicator_derivative_3D_1st(xlocan,x_inter,x_center,&
                                I_fluid_center,corr_Ip,II,dI,ddI,norm_p,curv_p)
        end if

	if(intflag==1) then
          if( (abs(II-0.5).lt.0.1) .and. (abs(II-0.5).gt.0.03)) then
	    nn_inter_regen=nn_inter_regen+1
	    x_inter_regen(1:nsd,nn_inter_regen)=xlocan(1:nsd)
	  end if
	elseif(intflag==2) then
	  if( (II.gt.0.5+0.03) .and. (II.lt.0.5+0.1)) then
	    nn_inter_regen=nn_inter_regen+1
	    x_inter_regen(1:nsd,nn_inter_regen)=xlocan(1:nsd)
	  end if
	elseif(intflag==3) then
	  if( (II.lt.0.5-0.03) .and. (II.gt.0.5-0.1)) then
	    nn_inter_regen=nn_inter_regen+1
	    x_inter_regen(1:nsd,nn_inter_regen)=xlocan(1:nsd)
	  end if
	end if
...
!goto 900
  ...
  do loc_index=1,nn_loc
    ...
    if(err_p.lt.1.0e-6) then
       if(nsd==2) then
              call get_curv_num_2D(xlocan,x_inter,x_center,I_fluid_center,&
                        corr_Ip,dcurv,curv_p,norm_p)
       else if(nsd==3) then
              call get_curv_num_3D(xlocan,x_inter,x_center,I_fluid_center,&
                        corr_Ip,dcurv,curv_p,norm_p)
