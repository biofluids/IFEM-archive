...
!
       ...
       subroutine getinf_el_solid(finf, x, xna, nn, nsd, ne, nen, ien, maxconn)
	   implicit none
	   integer :: ninf, nn, nsd, ne, nen, maxconn ! fluid variable from main
	   integer ien(ne,nen) ! transfer matrix connectted local information to global
	  ...
	  if(ninf == 0) goto 2000
	   ...
	   if (nsd==2 .and. nen==3) then
	      do i= 1, ninf
				   xna_p1(1:nsd) = xna(1:nsd, ien(inf(i),1))
				   xna_p2(1:nsd) = xna(1:nsd, ien(inf(i),2))
				   xna_p3(1:nsd) = xna(1:nsd, ien(inf(i),3))
          ...
          if (nsd==2 .and. nen==4) then
                 ...
                 ! write(*,*) 'xna= ',i, xna(1:nsd,ien(1:nen,inf(i)))
                  vector_fe1(1:nsd)=xna(1:nsd,ien(inf(i),2))-xna(1:nsd,ien(inf(i),1))
                  vector_fe2(1:nsd)=xna(1:nsd,ien(inf(i),3))-xna(1:nsd,ien(inf(i),1))
                  vector_p(1:nsd)=x(1:nsd)-xna(1:nsd,ien(inf(i),1))
                 ...
                 ! write(*,*) '1-2', sign_1, sign_2
                  ...
                  !1-2
                  vector_fe1(1:nsd)=xna(1:nsd,ien(inf(i),3))-xna(1:nsd,ien(inf(i),2))
                  vector_fe2(1:nsd)=xna(1:nsd,ien(inf(i),4))-xna(1:nsd,ien(inf(i),2))
                  vector_p(1:nsd)=x(1:nsd)-xna(1:nsd,ien(inf(i),2))
                  sign_1=vector_fe1(1)*vector_fe2(2)-vector_fe1(2)*vector_fe2(1)
                  sign_2=vector_fe1(1)*vector_p(2)-vector_fe1(2)*vector_p(1)
                  sign_fe=sign_2*sign_1
                 ! write(*,*) '2-3', sign_1, sign_2
                  ...
                  vector_fe1(1:nsd)=xna(1:nsd,ien(inf(i),4))-xna(1:nsd,ien(inf(i),3))
                  vector_fe2(1:nsd)=xna(1:nsd,ien(inf(i),1))-xna(1:nsd,ien(inf(i),3))
                  vector_p(1:nsd)=x(1:nsd)-xna(1:nsd,ien(inf(i),3))
                  sign_1=vector_fe1(1)*vector_fe2(2)-vector_fe1(2)*vector_fe2(1)
                  sign_2=vector_fe1(1)*vector_p(2)-vector_fe1(2)*vector_p(1)
                  sign_fe=sign_2*sign_1
                 ! write(*,*)'3-4',sign_1,sign_2
                  ...
                  vector_fe1(1:nsd)=xna(1:nsd,ien(inf(i),1))-xna(1:nsd,ien(inf(i),4))
                  vector_fe2(1:nsd)=xna(1:nsd,ien(inf(i),2))-xna(1:nsd,ien(inf(i),4))
                  vector_p(1:nsd)=x(1:nsd)-xna(1:nsd,ien(inf(i),4))
                  sign_1=vector_fe1(1)*vector_fe2(2)-vector_fe1(2)*vector_fe2(1)
                  sign_2=vector_fe1(1)*vector_p(2)-vector_fe1(2)*vector_p(1)
                  sign_fe=sign_2*sign_1
