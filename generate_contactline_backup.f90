...
subroutine generate_contactline(x_inter_regen,nn_inter_regen,x,x_inter,x_center,hg,I_fluid_center, &
  ...
  real(8) x_inter_regen(nsd,maxmatrix),x(nsd,nn),x_inter(nsd,maxmatrix),x_center(nsd,ne),hg(ne)
  integer nn_inter_regen
  real(8) I_fluid_center(ne),corr_Ip(maxmatrix)
  integer ien(nen,ne),nn_con_ele,con_ele(nn_con_ele)
  real(8) norm_con_ele(nsd,nn_con_ele),vel_fluid(nsd,nn)
...
write(*,*)'inside generate constactline',myid
goto 999
  NumRegen=0
  IndexRef=0
  x_cor_regen(:,:)=0.0
  ...
  do loc_index=1,nn_loc
	...
	if(err_p.lt.1.0e-6) then
	  ...
	  NumRegen=icount
	  if( (xlocan(1)-xc(1))*norm_e(1)+(xlocan(2)-xc(2))*norm_e(2)+(xlocan(3)-xc(3))*norm_e(3).lt.0.5*max_hg) then
	      IndexRef=icount
	      norm_r(:)=norm_g(:)
	      xr(:)=xlocan(:)
	  end if
...
write(*,*)'before flag_ca'
	 ...
	 do jcount=1,NumRegen
	    ...
	    x_contact_proc(:,nn_contact_proc)=x_cor_regen(:,j)
	 end do
      else
!	 call find_ca(xr,x,vel_fluid,vol_nn,ca,norm_r,thelta,anglet,flag_ca)
      end if
