...
!          nxcan=nxcan+1
	  ...
	  if(flag==1) then
	    call find_ca(xlocan,x,vel_fluid,vol_nn,ca,norm_p,thelta)
	    if(norm_p(1).lt.0) then
	      norm_wall(1)=cos(thelta+pi/2.0)
	    else
	      norm_wall(1)=cos(pi/2.0-thelta)
	    end if
	    if(flag_n==1) then
	       norm_wall(2)=sin(thelta+pi/2.0)
	    else
	       norm_wall(2)=-sin(thelta+pi/2.0)
	    end if
	    x_fix(:)=xlocan(:)
	    norm_fix(:)=norm_p(:)
	    if(myid==0)write(*,*)'norm_wall=',norm_wall(1:nsd)
	    if(flag_n==1) then
              x_wall(2)=-0.5
	    else
	      x_wall(2)=0.5
