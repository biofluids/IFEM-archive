...
subroutine gmres_ale(x,d,dold,w,bg,dg,hg,ien,fext,id,meshvel, &
...
! ???????????????????????????????????????????????????????????
		...
		call blockgmres_ale(x,vloc,dold,av_tmp,hg,ien,fext,meshvel,ne_local,ien_local,node_local,nn_local)
...
!        if (myid == 0) write(*,*) 'ALE BLOCK time', end_time - start_time
...
!		call vector_dot_pa(avloc,Vm(:,i),ndf,nn,nn_local,node_local,Hm(i,j))
		! Do the vector product of v_i * v_i set it to be h_i,j
                ...
                Hm(1:j,j)=space2(1:j)
!               if (myid == 0) write(*,*) 'Hm0 = ', Hm(:,:)
        call date_and_time(values=time_arrary_1)
        start_time=time_arrary_0(5)*3600+time_arrary_0(6)*60+time_arrary_0(7)+time_arrary_0(8)*0.001
        end_time=time_arrary_1(5)*3600+time_arrary_1(6)*60+time_arrary_1(7)+time_arrary_1(8)*0.001
!        if (myid == 0) write(*,*) 'ALE Communication time', end_time - start_time
