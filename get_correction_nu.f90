...
!  real(8) x(nsd,nn),I_fluid(nn)
  ...
  real(8) dx(nsd),hs,Sp
  real(8) A(nn_inter,length),A_temp(nn_inter,length)
  integer A_index(nn_inter,length),A_index_temp(nn_inter,length)
  integer nn_count(nn_inter),nn_count_temp(nn_inter)
...
write(*,*)'i am here'
  do loc_index=1,nn_inter_loc
	...
	ncount=ncount+1
if(ncount.gt.length) write(*,*)'enlarge get_corretion_nu',ncount

	A_temp(i,ncount)=Sp*temp
	A_index_temp(i,ncount)=j
	if(j==i) then
	  w_temp(i)=A_temp(i,ncount)
	end if
	end if
     end do

     nn_count_temp(i)=ncount   
     do j=1,nn_center_domain
	ie=center_domain(j)
	hsg=hg(ie)
	dx(:)=abs(x_inter(:,i)-x_center(:,ie))
  ...
  call mpi_allreduce(A_temp(1,1),A(1,1),nn_inter*length,mpi_double_precision, &
                mpi_sum,mpi_comm_world,ierror)
  call mpi_allreduce(A_index_temp(1,1),A_index(1,1),nn_inter*length,mpi_integer, &
                mpi_sum,mpi_comm_world,ierror)
  call mpi_allreduce(nn_count_temp(1),nn_count(1),nn_inter,mpi_integer, &
                mpi_sum,mpi_comm_world,ierror)
  ...
  call mpi_barrier(mpi_comm_world,ierror)
if(myid==0)write(*,*)w(1:nn_inter)
     BB(:)=0.5-BB(:)
!     call gmres_correction(A,BB,nn_inter)
if(myid==0)write(*,*)'begin gmres'
     call gmres_correction(A,A_index,BB,w,nn_inter,length,nn_count,nn_inter_loc)
