subroutine r_spiola_elastic2(det,xot,ge,iq,ne,cstr_element)
  ...
  real(8) :: young_mod2, Poisson2 !Lucy Zhang added

  cstr(:,:) = 0.0d0
  
! Lucy Zhang added for second material type. Manually change Poisson's ratio and Young's Modulus. 
  Poisson2=0.3
  young_mod2=10000 
! -------------------
  threedim: if (nsd_solid .eq. 3) then 

  tempC= young_mod2*(1-Poisson2)/((1+Poisson2)*(1-2*Poisson2))
  tempD= Poisson2/(1-Poisson2)
  tempE= (1-2*Poisson2)/(2*(1-Poisson2))
