subroutine communicate_res_ad_subsolid(res,ndf,nn,send_address,ad_length)
use mpi_variables
implicit none
include 'mpif.h'
! inputs
...
integer tag
integer size
integer reqr(countrow_solid)
integer reqs(countrow_solid)
integer status(mpi_status_size,countrow_solid)
...
do icount=1,countrow_solid
	i=sub_address_solid(icount,2) ! get envelope size
	des=sub_address_solid(icount,1) ! get envelope destination
...
call mpi_waitall(countrow_solid,reqs,status,ierror)
call mpi_waitall(countrow_solid,reqr,status,ierror)
...
end subroutine communicate_res_ad_subsolid
