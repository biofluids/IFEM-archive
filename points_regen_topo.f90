...
subroutine points_regen_topo(x,x_inter,x_center,x_inter_regen,nn_inter_regen,&
			I_fluid_center,corr_Ip,hg,ien,intflag,nn_con_ele,con_ele,norm_con_ele)
  ...
  integer finf,maxconn,index_con
...
!           if(((II-Ic_inter).gt.0.).or.((Ic_inter-II).gt.0.02)) then
	    if( (abs(II-Ic_inter).gt.0.15) .or. (abs(II-Ic_inter).lt.0.1) .or. ((II-Ic_inter).gt.0)) then
                goto 200
           end if
       else if(intflag==3) then

            if( (abs(II-Ic_inter).gt.0.15) .or. (abs(II-Ic_inter).lt.0.1) .or. ((II-Ic_inter).lt.0)) then
                goto 200
           end if
...
!==========================================================!
          call getinf_el_3d_con(finf,xlocan,x,nn,nsd,ne,nen,ien,maxconn,nn_con_ele,con_ele,index_con)
           if(finf.ne.0) then
             temp=(xlocan(1)-x_center(1,finf))*norm_con_ele(1,index_con)+(xlocan(2)-x_center(2,finf))*norm_con_ele(2,index_con)
             if(temp.lt.-max_hg/10.0) goto 200
           end if
...
nn_inter_regen_loc=nn_inter_regen
x_regen_temp=x_inter_regen
nn_inter_regen=0
do icount=1,nn_inter_regen_loc
   xlocan(:)=x_regen_temp(:,icount)
   call getinf_el_3d_con(finf,xlocan,x,nn,nsd,ne,nen,ien,maxconn,nn_con_ele,con_ele,index_con)
   if(finf.ne.0) then
      temp=(xlocan(1)-x_center(1,finf))*norm_con_ele(1,index_con)+(xlocan(2)-x_center(2,finf))*norm_con_ele(2,index_con)
      if(temp.lt.hg(finf)*0.4) then
         I_fluid_center(finf)=0.55
          goto 666
       end if
   end if
...
end subroutine points_regen_topo
