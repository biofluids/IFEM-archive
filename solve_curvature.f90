...
subroutine solve_curvature(x,x_inter,x_center,I_fluid,corr_Ip,I_fluid_center,curv_nn,ien,&
  ...
  real(8) corr_Ip(maxmatrix),I_fluid_center(nn_center),curv_nn(nn)
  integer ien(nen,ne)
...
!if(x(3,i).lt.0.001) goto 888
	...
	if(nsd==2) then
	   call get_indicator_derivative_2D(xlocan,x_inter,x_center,I_fluid_center, &
			corr_Ip,II,dI,ddI,norm_a,curv_a)
	else
           call get_indicator_derivative_3D(xlocan,x_inter,x_center,I_fluid_center, &
                        corr_Ip,II,dI,ddI,norm_a,curv_a)
	end if
	dis=0.0
	do isd=1,nsd
...
do i=1,nn
  ...
  call gmrescurv(x,w,p,dg,ien,id_curv,&
                  ne_local,ien_local,node_local,nn_local, &
                   global_com,nn_global_com,local_com,nn_local_com,send_address,ad_length,&
