...
subroutine contactline_y(x_inter,x,norm_inter,conele,ne_conele,x_center,hg,&
...
!  do i=1,nn_inter
!     if(abs(x_inter(1,i)).lt.abs(x_limit)) then
...
!  end do
  ...
  min_x=minval(x_inter(1,1:nn_inter))
  max_x=maxval(x_inter(1,1:nn_inter))
!  x_wall(2,:)=-0.5
if((min_x.lt.-0.5+2.0*max_hg).and.(max_x.gt.0.5-2.0*max_hg)) then
  do i=1,nn_inter
     if(abs(x_inter(1,i)).lt.abs(x_limit)) then
       nn_inter_temp=nn_inter_temp+1
       x_inter_temp(:,nn_inter_temp)=x_inter(:,i)
...
if((min_x.lt.-0.5+2.0*max_hg).and.(max_x.lt.0.5-2.0*max_hg)) then
  do i=1,nn_inter
     if(x_inter(1,i).gt.x_limit) then
       nn_inter_temp=nn_inter_temp+1
       x_inter_temp(:,nn_inter_temp)=x_inter(:,i)
     end if
  end do
...
if((min_x.gt.-0.5+2.0*max_hg).and.(max_x.gt.0.5-2.0*max_hg)) then
  do i=1,nn_inter
     if(x_inter(1,i).lt.0.5-2.0*max_hg) then
       nn_inter_temp=nn_inter_temp+1
       x_inter_temp(:,nn_inter_temp)=x_inter(:,i)
     end if
  end do
...
do flag_n=1,2
  if(flag_n==1) then
    if(minval(x_inter(1,1:nn_inter)).gt.-0.5*2.0*max_hg) goto 300
    x_limit=-0.5+2.5*max_hg
  else
    if(maxval(x_inter(1,1:nn_inter)).lt.0.5-2.0*max_hg) goto 300
    x_limit=0.5-2.5*max_hg
  end if
  nxcan=0
...
!          nxcan=nxcan+1
	  xlocan(:)=0.0
	  do inl=1,nen
	     xlocan(2)=xlocan(2)+1.0/real(nen)*x(2,ien(inl,conele(i,flag_n)))
	  end do
	  xlocan(1)=x_limit
	  nit=1
	  err_p=999.0
	  ...
	  flag=1
	  do icount=1,nxcan-1
	     if(abs(xcan(2,nxcan)-xcan(2,icount)).lt.max_hg) then
	       flag=0
	     end if
	  end do
	  if(flag==1) then
	    if(norm_p(2).lt.0) then
...
300 continue
  ...
  do i=1,nn_con_regen
  if(  (abs(x_con_regen(1,i)).lt.0.5) .and. (abs(x_con_regen(2,i)).lt.0.5))then
     nn_inter=nn_inter+1
!     x_inter(:,nn_inter)=x_con_regen(:,i)
      x_inter(1,nn_inter)=x_con_regen(2,i)
       x_inter(2,nn_inter)=x_con_regen(1,i)
  end if
  end do

end subroutine contactline_y
