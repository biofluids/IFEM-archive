...
subroutine block(xloc, dloc, doloc, p, q_p, hk, ien, f_fluids,rngface, f_stress,&
		ne_local,ien_local, node_local,nn_local,fden,fvis,I_solid)
...
!-----------------------------------------------------------------------------
		node=ien(inl,ie)
		local_den(inl)=(dloc(ndf,node)+P0)/(ZC*RC*TC)*(1.0-I_solid(node))+&
				(density_solid+den_liq)*I_solid(node)
...
!==================for compressible flow=========================
		     ...
		     res_c=res_c+(sh(xsd,inl)*d(udf,inl)+sh(ysd,inl)*d(vdf,inl))+ &
			  (u*sh(xsd,inl)*d(pdf,inl)+v*sh(ysd,inl)*d(pdf,inl))/(pp+P0)*(1.0-I_solid(node))
		     q_res_c(inl)=(sh(0,inl)*dtinv+u*sh(xsd,inl)+v*sh(ysd,inl))/(pp+P0)*(1.0-I_solid(node)) &
				  +sh(0,inl)*dtinv/kappa*I_solid(node)
		...
		elseif (nsd==3) then
		  do inl=1,nen
		     ...
		     res_c = res_c+sh(xsd,inl)*d(udf,inl) &
			    ...
			    /(pp+P0)*(1.0-I_solid(node))

		     q_res_c(inl)=(sh(0,inl)*dtinv+u*sh(xsd,inl)+v*sh(ysd,inl)+w*sh(zsd,inl))/(pp+P0)*(1.0-I_solid(node)) &
		     +sh(0,inl)*dtinv/kappa*I_solid(node)
		  enddo
		endif
		...
		do inl=1,nen
		   node=ien(inl,ie)
		   temp=temp+sh(0,inl)*((d(ndf,inl)+P0)*(1.0-I_solid(node))+kappa*I_solid(node))
		end do
		do inl=1,nen
		   node=ien(inl,ie)
		   res_c=res_c+sh(0,inl)*(d(ndf,inl)-d_old(ndf,inl))*dtinv* &
			(1.0/(pp+P0)*(1.0-I_solid(node))+1.0/kappa*I_solid(node))
