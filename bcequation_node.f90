subroutine bcequation_node(xloc,dloc,ien,ginflow,linflow,nn_inflow,rng,bdel,ne_inflow,bdindex,pin,vel_bc,error)
...
! input arguments
 ...
 real(8) error ! error sign
 real(8) vel_bc(nsd,nn_inflow) ! Boundary velocity outputs
!===========================================
...
integer jnl
integer info
integer ipiv(nn_inflow*2)
real(8) error_tmp(nn_inflow*nsd)
real(8) pin_set 
!======================================
...
do ia=1,n_edge
	iq=li(ia)
! iq is the node index of lacal element
call bc_shape2d_node(sq,nsd,nen) ! get shape function in local sapce
...
fx=0.0d0
fy=0.0d0
pin_set=pin-d(3,iq) ! modify this as Heywood's 1996 paper
pin_set=-pin_set        ! This is change sign for the right BC since the outward normal towards -x direction
do inl=1,nen
...
fx=pin_set/(vis_liq/den_liq)-fx ! fx= P/(nu) - N_3,x * u_3 + N_4,x * u_4 (Also see notes)
...
ix=ien(iq,ie) ! global node index for the node on the boundary
...
a_lhs(1,3)=-1.0d0 ! artificial BC for bcequation
...
a_lhs(nn_inflow*2-1,(nn_inflow-1)*2-1)=-1.0d0 !artificial BC for bcequation
...
neq=nn_inflow*2
call DGESV(neq,1,a_lhs,neq,ipiv,f_rhs,neq,info)

do ja=1,nn_inflow
	vel_bc(1,ja) = f_rhs(ja*2-1)
	vel_bc(2,ja) = f_rhs(ja*2)
do ia=1,nsd
        error_tmp((ja-1)*ia+1) =abs( vel_bc(ia,ja) - dloc(ia,linflow(ja)))
end do

end do

error=maxval(error_tmp)

open(unit=300, file='X', status='unknown')
