...
!  cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine block_com(xloc, dloc, doloc, p, q_p, hk, ien, f_fluids,rngface, f_stress, ne_local,ien_local,node_local,nn_local)
...
!....  calculate each term in the residual equation
		...
		if (nsd==2) then
		  do inl=1,nen
		  	 ...
		  	 q_res_c(inl) = sh(0,inl)*dtinv/com_kappa! get res_c for P for continuity equation
		  enddo
		elseif (nsd==3) then
		  do inl=1,nen
		     res_c = res_c+sh(xsd,inl)*d(udf,inl) &
			 ...
			 q_res_c(inl) = sh(0,inl)*dtinv/com_kappa
		  enddo
		endif
...
!				diag(inl)=p(1,node)
			  enddo
			       ...
			       q_p(1,node)=q_p(1,node)+ph(1,inl)*mu*(sh(1,inl)*q_d(1,inl)*(2-2/3))+&
					     ph(2,inl)*mu*sh(2,inl)*q_d(1,inl)
			       q_p(2,node)=q_p(2,node)+ph(1,inl)*mu*sh(1,inl)*q_d(2,inl)+ &
					     ph(2,inl)*(mu*sh(2,inl)*q_d(2,inl)*(2-2/3))
			...
			elseif (nsd==3) then
			  do isd=1,nsd
				p(isd,node)=p(isd,node) + ph(isd,inl)*pp -   &
										  ...
										  ph(2,inl)*tau(2,isd) -  &
										  ph(3,inl)*tau(3,isd)
				p(isd,node)=p(isd,node)+mu*ph(isd,inl)*(dr(1,1)+dr(2,2)+dr(3,3))*2/3
			  enddo
                          q_p(1,node)=q_p(1,node)+ph(1,inl)*mu*(sh(1,inl)*(2-2/3))+ph(2,inl)*mu*sh(2,inl)+ph(3,inl)*mu*sh(3,inl)
                          q_p(2,node)=q_p(2,node)+ph(1,inl)*mu*sh(1,inl)+ph(2,inl)*mu*(sh(2,inl)*(2-2/3))+ph(3,inl)*mu*sh(3,inl)
                          q_p(3,node)=q_p(3,node)+ph(1,inl)*mu*sh(1,inl)+ph(2,inl)*mu*sh(2,inl)+ph(3,inl)*mu*(sh(3,inl)*(2-2/3))
