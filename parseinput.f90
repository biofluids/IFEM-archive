...
if (myid ==0) then
  ...
  if (material_type==2) write(*,*) 'The solid is LINEAR ELASTIC material; parallel solver'
  if (material_type==12) write(*,*) 'The solid is LINEAR ELASTIC material; serial solver'
...
!        write(*,*) 'fix', fix(:)
        ...
        do idf=1,ndf
            ...
            if (bv(idf,ibc) .lt. -80000.0) then
                ...
                !bc(idf,ibc) = 1
                !bv(idf,ibc) = 0.0
            elseif (bv(idf,ibc) .gt. 80000.0) then
                xyzcPML(ibc) = bv(idf,ibc)-90000.0
                flagPML(ibc) = idf
                sumNbcPML = sumNbcPML + 1
                nbcTimeVar = ibc
