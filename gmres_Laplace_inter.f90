...
subroutine gmres_Laplace_inter(x,w,bg,dg,ien,node_id,ele_id, &
	...
	integer,parameter :: ndf = 1
        integer,parameter :: inner=100
        integer,parameter :: outer=5
	real* 8 x(nsd,nn)
	integer node_id(ndf*nn),ele_id(ne)
	integer ien(nen,ne)
...
call blockgmres_Laplace_inter(x,av_tmp,ien,ne_local,ien_local,vloc,ele_id)
...
!==================
		call equal_pa(av_tmp,avloc,ndf,nn,node_local,nn_local)
                ...
                call setid_pa(avloc,ndf,nn,node_id,node_local,nn_local)
                space1(:)=0.0d0
                space2(:)=0.0d0
	      do i=1,j
		 do icount=1,nn_local
