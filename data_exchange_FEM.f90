...
integer ipiv(nen)
real(8) work(nen)
real(8) me_lap(nen,nen)
!real(8) me1_tmp(nen,nen)
        do i=1,nen
        sh1(i)=0
        do j=1,nen
        me(i,j)=0
        ...
        if (nsd .eq. 2) then  ! 2-D case
           ...
           do i=1,nen
           me1(1,i)=1.0
           me1(2,i:(nsd+1))=xe(1:nsd,i)
           end do
!	me1_tmp(:,:)=me1(:,:)
!                call DGETRF(nen,nen,me1,nen,ipiv,info)
!		det=1.0d0
!		do i=1,nen
!			det=det*me1(i,i)
!		end do
           call determinant(me1_tmp,nen,nen,det)
          ! write(*,*) det
