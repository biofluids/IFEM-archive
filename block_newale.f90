...
!  cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine block_newale(xloc, dloc, doloc, p, q, hk, ien, f_fluids,rngface, f_stress,meshvel)
...
!============================================================================
		 ...
		 meshvel_loc(1:nsd,inl) = mesh_vel(1:nsd,ien(inl,ie))
...
!... calculate vi, dvi/dxj
        do inl=1,nen
		   tempc(1:nsd) = ama*d(1:nsd,inl)+oma*d_old(1:nsd,inl)-meshvel_loc(1:nsd,inl)
		   drs(1:nsd) = drs(1:nsd)+sh(0,inl)*tempc(1:nsd)
		   do isd=1,nsd
                       !  dr(isd,1:nsd) = dr(isd,1:nsd)+sh(isd,inl)*tempc(1:nsd)
			 dr(isd,1:nsd) = dr(isd,1:nsd)+sh(isd,inl)*d(1:nsd,inl)
		   enddo
