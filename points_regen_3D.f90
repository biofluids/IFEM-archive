...
subroutine points_regen_3D(x,x_inter,x_center,x_inter_regen,nn_inter_regen,&
  ...
  integer i,j,k,isd,inl,node,ie,icount,jcount
...
!******************************************************
  do jcount=1,ne_regen_ele_loc !begin element loop
	...
	x_fluid(1:nsd,inl)=x(1:nsd,node)
     ...
     nn_sub=4
!!!!!!**************************************************
     do while((nn_local.le.nn_cr).and.(nn_sub.le.4)) !begin regeneration loop
	nn_local=0
	x_inter_ele_loc(:,:)=0.0
...
!!!!!!!!!**********************************************
	   ...
	   if(nsd==3 .and. nen==8) then
	     sh(1)=0.125*(1-x_loc_can(1,i))*(1-x_loc_can(2,i))*(1-x_loc_can(3,i))
	     sh(2)=0.125*(1+x_loc_can(1,i))*(1-x_loc_can(2,i))*(1-x_loc_can(3,i))
	     sh(3)=0.125*(1+x_loc_can(1,i))*(1+x_loc_can(2,i))*(1-x_loc_can(3,i))
	     sh(4)=0.125*(1-x_loc_can(1,i))*(1+x_loc_can(2,i))*(1-x_loc_can(3,i))
	     sh(5)=0.125*(1-x_loc_can(1,i))*(1-x_loc_can(2,i))*(1+x_loc_can(3,i))
	     sh(6)=0.125*(1+x_loc_can(1,i))*(1-x_loc_can(2,i))*(1+x_loc_can(3,i))
	     sh(7)=0.125*(1+x_loc_can(1,i))*(1+x_loc_can(2,i))*(1+x_loc_can(3,i))
	     sh(8)=0.125*(1-x_loc_can(1,i))*(1+x_loc_can(2,i))*(1+x_loc_can(3,i))
	   end if
           xlocan(:)=0.0
           do inl=1,nen
...
!           if(((II-Ic_inter).gt.0.).or.((Ic_inter-II).gt.0.02)) then
	    if( (abs(II-Ic_inter).gt.0.1) .or. (abs(II-Ic_inter).lt.0.01) .or. ((II-Ic_inter).gt.0)) then
                goto 200
           end if

       else if(intflag==1) then
!           if((II.gt.Ic_inter+0.25).or.(II.lt.Ic_inter-0.25).or.(abs(II-Ic_inter).lt.1.0e-3)) then
            if( (abs(II-Ic_inter).gt.0.1) .or. (abs(II-Ic_inter).lt.0.01)) then
...
!                   dcurv=0.1
	   ...
	   if(flag_loc==1) then
			...
			(x_inter_ele_loc(3,icount)-xlocan(3))**2
                temp=sqrt(temp)
...
if(myid==0) then
  ...
  if(nn_inter_regen.gt.maxmatrix) write(*,*)'exceed max matrix limit'
...
end subroutine points_regen_3D
