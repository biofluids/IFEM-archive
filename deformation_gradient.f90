...
!       Lucy Zhang,
!       Calculate deformation gradient for ALE.  7/1/99
!       cccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine defgrad(xloc, xrefloc,ien,f,jac,finv,jacinv)
    use fluid_variables
    implicit none

    real(8),intent(in)  :: xloc(nsd,nn), xrefloc(nsd,nn)
    integer,intent(in)  :: ien(nen,ne)
    real(8),intent(out) :: f(nsd,nsd,nquad,ne)
    real(8),intent(out) :: jac(nquad,ne)
    real(8),intent(out) :: finv(nsd,nsd,nquad,ne)
    real(8),intent(out) :: jacinv(nquad,ne)

    real(8) :: x(nsdpad,nenpad)
    real(8) :: det, minor(nsd)
    ...
    integer :: inl, ie, isd, iq
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

!.... calculate d(Na)/d(xref)
    do ie=1,ne
       do inl=1,nen
          do isd=1,nsd
...
!c.... calculate the first derivative, d(x)/d(xref)
          ...
          jacinv(iq,ie)=finv11 *minor(1) + finv12 *minor(2) + finv13 *minor(3)
       enddo
    enddo
    return
...
!.... calculate the first derivative, d(xref)/d(x)
          do inl=1,nen
             do isd=1,nsd
          ...
          jacinv(ie)=finv(1,1,ie)*minor(1)+finv(1,2,ie)*minor(2)+finv(1,3,ie)*minor(3)
       enddo
    enddo

    return

end subroutine defgradinv


end module deformation_gradient