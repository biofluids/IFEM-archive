	...
	do jcount=1,nsd+1
	   ...
	   M(icount,jcount,8)=M(icount,jcount,8)+(Mvec(icount,jcount,8)*Phi(1,j)+ &
           ...
           M(icount,jcount,10)=M(icount,jcount,10)+(Mvec(icount,jcount,10)*Phi(1,j)+ &
                                                  ...
                                                  Mvec(icount,jcount,1)*Phi(10,j))/(hsp**nsd)*(hsg**nsd)
	end do
    end do
   end if  !!!!!!!
  end do
...
!====construct vec vecx vecy vecxx vecyy vecxy==========
	...
	dtemp(8)=dtemp(8)+vec(icount,8)*B(icount,1)+vec(icount,2)*B(icount,3)+ &
     ...
     W1(3)=(dtemp(4)*Phi(1,j)+dtemp(1)*Phi(4,j))/(hsp**nsd)*(hsg**nsd)
     W2(1)=(dtemp(5)*Phi(1,j)+2*dtemp(2)*Phi(2,j)+dtemp(1)*Phi(5,j))/(hsp**nsd)*(hsg**nsd)
     W2(2)=(dtemp(6)*Phi(1,j)+2*dtemp(3)*Phi(3,j)+dtemp(1)*Phi(6,j))/(hsp**nsd)*(hsg**nsd)
     W2(3)=(dtemp(7)*Phi(1,j)+2*dtemp(4)*Phi(4,j)+dtemp(1)*Phi(7,j))/(hsp**nsd)*(hsg**nsd)
     W2(4)=(dtemp(8)*Phi(1,j)+dtemp(2)*Phi(3,j)+dtemp(3)*Phi(2,j)+dtemp(1)*Phi(8,j))/(hsp**nsd)*(hsg**nsd)
     W2(5)=(dtemp(9)*Phi(1,j)+dtemp(2)*Phi(4,j)+dtemp(4)*Phi(2,j)+dtemp(1)*Phi(9,j))/(hsp**nsd)*(hsg**nsd)
     W2(6)=(dtemp(10)*Phi(1,j)+dtemp(3)*Phi(4,j)+dtemp(4)*Phi(3,j)+dtemp(1)*Phi(10,j))/(hsp**nsd)*(hsg**nsd)
...
!     end do
     ...
     W2(5)=(dtemp(8)*Phi_inter(1,j)+dtemp(2)*Phi_inter(3,j)+dtemp(3)*Phi_inter(2,j)+dtemp(1)*Phi_inter(8,j))!/(hsp**nsd)*(hsg**nsd)
     W2(6)=(dtemp(9)*Phi_inter(1,j)+dtemp(2)*Phi_inter(4,j)+dtemp(4)*Phi_inter(2,j)+dtemp(1)*Phi_inter(9,j))
     W2(5)=(dtemp(10)*Phi_inter(1,j)+dtemp(3)*Phi_inter(4,j)+dtemp(4)*Phi_inter(3,j)+dtemp(1)*Phi_inter(10,j))
       II=II+W0*corr_Ip(j)
       dI(1:nsd)=dI(1:nsd)+W1(1:nsd)*corr_Ip(j)
       ddI(1:nsd)=ddI(1:nsd)+W2(1:nsd)*corr_Ip(j)
