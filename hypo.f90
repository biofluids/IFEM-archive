...
! calculate \sigma_x and \sigma_y for PML... and, other things lol
    ...
    !include "initialConditionFluid.fi"
    d(:,:)=0.0
!-----------------------------------------------------------------
!=============================
...
!=================================================================
    time_loop: do its = nts_start,nts !.....count from 1 or restart-timestep to number of timesteps
        ...
        ChoiceInterpolation: if (ndelta==1) then
            ...
            StepsSolvingSolid: if (its > 0) then
                if (myid == 0) write(*,*) '=== Fluid solver have converged for', its,'time steps ==='
                time = mpi_wtime()
                id_solidbc(:,:)=1
                OptionSolveSolid: if (material_type .ne. 999) then
            ...
            if (material_type /= 999) then
                ...
                !elseif ((material_type == 12) .and. (myid == 0)) then
                elseif (material_type == 12) then
            ...
            !=================================================================
            ! Solid solver
            if ((myid == 0) .and. (material_type /= 999)) then
                write(*,*) 'starting my own little solid solver'
