...
subroutine indicator_denmesh_re(I_fluid,I_fluid_den,x_den,ien_den,bcnode_den,its)
...
!				den_domain,ne_den_domain
  use allocate_variables,only: regen_ele, ne_regen_ele, &
			       center_domain,nn_center_domain
!inter_ele_den(1:ne_inter_den),den_domain(1:ne_den_domain),inter_ele_den(1:ne_inter_den)
  ...
  if(its==1) then
	...
	flag_offdomain(center_domain(i))=1
    end do
    do i=1,ne_den
    ...
    fake_ne=nn_center_domain
    fake_domain(1:fake_ne)=center_domain(1:nn_center_domain)
...
!==============================================================
    dg(:)=0.0
    call gmres_Laplace_re(fake_ne,fake_domain,x_den,I_fluid_den,w,p,dg,ien_den,bcnode_den,&
...
end subroutine indicator_denmesh_re
