...
c  S. Aliabadi                                                          c
c  cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	  subroutine blockgmres(xloc,shrk,don,doon,qon,p,hk,
	1	   ien,rng,cnn,ncnn)
      ...
      real* 8 p(ndf,nn_on),qon(ndf,nn_on),hk(nec)

      real* 8 x(nsdpad,nenpad)
      real* 8 d(ndfpad,maxconn),do(ndfpad,maxconn),q(ndfpad,maxconn)
...
c  coef = vis_liq/sqrt(hmin*hmin + hmax*hmax)
	  ...
	  do ie=1,nec 
		...
		do iq=1,nquad
		  ...
		  enddo
		  do inl=1,ncnn(qp)
			qrs(udf)=qrs(udf)+shrk(0,inl,qp)*q(udf,inl)      
			qrx(udf)=qrx(udf)+shrk(1,inl,qp)*q(udf,inl)      
			qry(udf)=qry(udf)+shrk(2,inl,qp)*q(udf,inl)      
			qrz(udf)=qrz(udf)+shrk(3,inl,qp)*q(udf,inl)      
			qrs(vdf)=qrs(vdf)+shrk(0,inl,qp)*q(vdf,inl)      
			qrx(vdf)=qrx(vdf)+shrk(1,inl,qp)*q(vdf,inl)      
			qry(vdf)=qry(vdf)+shrk(2,inl,qp)*q(vdf,inl)      
			qrz(vdf)=qrz(vdf)+shrk(3,inl,qp)*q(vdf,inl)      
			qrs(wdf)=qrs(wdf)+shrk(0,inl,qp)*q(wdf,inl)      
			qrx(wdf)=qrx(wdf)+shrk(1,inl,qp)*q(wdf,inl)      
			qry(wdf)=qry(wdf)+shrk(2,inl,qp)*q(wdf,inl)      
			qrz(wdf)=qrz(wdf)+shrk(3,inl,qp)*q(wdf,inl)      
			qrt(udf)=qrt(udf)+shrk(0,inl,qp)*q(udf,inl)*dtinv
			qrt(vdf)=qrt(vdf)+shrk(0,inl,qp)*q(vdf,inl)*dtinv
			qrt(wdf)=qrt(wdf)+shrk(0,inl,qp)*q(wdf,inl)*dtinv
			qrs(pdf)=qrs(pdf)+shrk(0,inl,qp)*q(pdf,inl)      
			qrx(pdf)=qrx(pdf)+shrk(1,inl,qp)*q(pdf,inl)      
			qry(pdf)=qry(pdf)+shrk(2,inl,qp)*q(pdf,inl)      
			qrz(pdf)=qrz(pdf)+shrk(3,inl,qp)*q(pdf,inl)      
		  end do
...
ccccccccccc
	...
	3		   + 2*drz(wdf)**2+(drz(vdf)+dry(wdf))**2)
		  ...
		  res_c = 0.0
		  do inl=1,ncnn(qp)
			res_c = res_c+(shrk(xsd,inl,qp)*q(udf,inl)
	1			 + shrk(ysd,inl,qp)*q(vdf,inl)
	2			 + shrk(zsd,inl,qp)*q(wdf,inl))/alpha
		  enddo
