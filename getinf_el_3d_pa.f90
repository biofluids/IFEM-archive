...
!
       ...
       subroutine getinf_el_3d_pa(finf, x, xna, nn, nsd, ne, nen, ien, maxconn, ne_intlocal, ien_intlocal)
	   implicit none
...
! Variables to make search code parallel
	   integer ne_intlocal
	   integer ien_intlocal(ne_intlocal)
	   integer ie_local ! loop variable
       
       ninf = 0 
     !  write(*,*) 'nen', nen
     !  write(*,*) '1000 el in get ', xna(:,124)
	   do ie_local = 1, ne_intlocal
		ie = ien_intlocal(ie_local)
	      do inl = 1, nen
