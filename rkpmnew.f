...
c  Calculate maximum range of geometry
      do isd = 1,nsd
        ...
        lcell(isd) = maxval(adist(isd,:))
        ncell(isd) = floor((xnamin(isd)-xnamax(isd))/lcell(isd))+1
      end do
...
c  Loop over nodes
      do inl = 1,nn
        nx = ceiling((xna(1,inl)-xnamin(1))/lcell(1))
        ny = ceiling((xna(2,inl)-xnamin(2))/lcell(2))
        nz = ceiling((xna(3,inl)-xnamin(3))/lcell(3))
