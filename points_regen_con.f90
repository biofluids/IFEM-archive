...
subroutine points_regen_con(x,x_inter,x_center,x_inter_regen,nn_inter_regen,&
			I_fluid_center,corr_Ip,hg,ien,intflag,ne_regen,ele_regen)
  ...
  if(ne_regen.le.ncpus) then
    if(myid+1.le.ne_regen) then
      nn_loc=1
    else
      nn_loc=0
    end if
  else
     base=floor(real(ne_regen)/real(ncpus))
     top=ne_regen-base*ncpus
...
!     ie=regen_ele_loc(jcount)
   do jcount=1,nn_loc
	...
	x_fluid(1:nsd,inl)=x(1:nsd,node)
     ...
     nn_sub=8
!!!!!!**************************************************
     do while((nn_local.le.nn_cr).and.(nn_sub.le.8)) !begin regeneration loop
	nn_local=0
	x_inter_ele_loc(:,:)=0.0
...
!           if(((II-Ic_inter).gt.0.).or.((Ic_inter-II).gt.0.02)) then
	    if( (abs(II-Ic_inter).gt.0.25) .or. (abs(II-Ic_inter).lt.0.2) .or. ((II-Ic_inter).gt.0)) then
                goto 200
           end if
       else if(intflag==3) then

            if( (abs(II-Ic_inter).gt.0.25) .or. (abs(II-Ic_inter).lt.0.2) .or. ((II-Ic_inter).lt.0)) then
                goto 200
           end if
...
!write(*,*)'xlocan before=',xlocan(1:nsd)
...
!write(*,*)'myid=',myid,'nn_inter_regen_loc=',nn_inter_regen_loc
  ...
  call mpi_barrier(mpi_comm_world,ierror)
if(myid==0)write(*,*)'**nn_inter_regen for contact line=',nn_inter_regen
111 format(f14.10,f14.10,f14.10)

end subroutine points_regen_con
