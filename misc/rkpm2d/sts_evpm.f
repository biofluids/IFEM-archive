c
      subroutine calsts_evpm(disp,d_disp,vel,
     &            sts_CH,sts_PK1,
...
c    Note:
c    
c    The following version is used deformation gradient
c    at T = (n + theta) * dt
c
c--------------------------------------------------------------
c
...
c                                        ! at T = n * dt (or last)
      ...
      real*8 sig_jrate(4),dLtan(4,4),sts_dot(4) 
c
c......... Local variables ..................
c
      real*8 F_matx(3,3),Finv_matx(3,3),detF,detFn
      real*8 Fth_matx(3,3),Fthinv_matx(3,3),detFth
      real*8 trace_CH,trace_KH
      real*8 Sb,Eb                ! (Sb := eff_sts_n; Eb := eff_ste_n)
      real*8 Ed,Es,G0,C1111,C1122,C2211,C2222,C1212
      real*8 C3333,C3311,C3322
      real*8 Eb1,gtem             ! (gtem := 1 + (Eb/Eb1)^2
      real*8 Eb0,sig0,bigN,gsoft  ! (gsoft:= sig0 (1+ Eb/Eb0)^N/gtem )
      real*8 rc                   ! (rc = xkc1^2 + xkc2^2
      real*8 Ebtot_c,Ebtot,dG_Eb  ! (dG_Eb = d gsoft / d Eb )
      real*8 dEbtot_Eb,xxii       ! (xxii = theta*dt*hh*dEbtot_Eb) 
      real*8 dEbtot_Sb,hh         ! (hh = p:L:p - dEbtot_Eb/dEbtot_Sb)
      real*8 reg                  ! (reg := Sb/gsoft)
      real*8 smm,sminv            ! (sminv := 1/smm)
c
c..............................................
c     
...
c
      n_three = 3
      eps1    = 1.0d-5  ! ( the criteria for Jacobian)
      tmp     = 1.0 - theta
c      
c----------------------------------------------------------
...
c.......(2)...............
c
            Fth_matx(1,1) = Fth_matx(1,1)
     &       + shpkdx(ip,ik) * (disp(1,ipt) - d_disp(1,ipt)*tmp)
            Fth_matx(1,2) = Fth_matx(1,2) 
     &       + shpkdy(ip,ik) * (disp(1,ipt) - d_disp(1,ipt)*tmp)
            Fth_matx(2,1) = Fth_matx(2,1) 
     &       + shpkdx(ip,ik) * (disp(2,ipt) - d_disp(2,ipt)*tmp)
            Fth_matx(2,2) = Fth_matx(2,2) 
     &       + shpkdy(ip,ik) * (disp(2,ipt) - d_disp(2,ipt)*tmp)
c
c.......(2)..............
c
...
c
         do i=1,3
            ...
            Fth_matx(i,i) = Fth_matx(i,i) + 1.0
         enddo
c         
c.........Find the Jacobian
c
        detF   = F_matx(1,1)*F_matx(2,2) 
     &	       - F_matx(1,2)*F_matx(2,1)
c
        detFth = Fth_matx(1,1)*Fth_matx(2,2) 
     &         - Fth_matx(1,2)*Fth_matx(2,1)
c
...
c
      ...
      Finv_matx(3,3) =   1.0
c
      Fthinv_matx(1,1) =   Fth_matx(2,2)/detFth
      Fthinv_matx(1,2) = - Fth_matx(1,2)/detFth 
      Fthinv_matx(2,1) = - Fth_matx(2,1)/detFth 
      Fthinv_matx(2,2) =   Fth_matx(1,1)/detFth 
      Fthinv_matx(3,3) =   1.0
c
c.....Store the Cauchy stress and Kirchhoff stress 
c
      detFn = detFth - detF*theta
      detFn = detFn/tmp
      do i=1,3
         do j=1,3
            sts_CH_n(i,j) = sts_CH(i,j)
	    sts_KH(i,j)   = detFn*sts_CH(i,j)
         end do
      end do
c      
c
...
c     11  22  33  12
c
      Dtot(1) = dvD(1,1)*Fthinv_matx(1,1) + dvD(1,2)*Fthinv_matx(2,1)
      Dtot(2) = dvD(2,1)*Fthinv_matx(1,2) + dvD(2,2)*Fthinv_matx(2,2)
      Dtot(3) = 0.0
      Dtot(4) = dvD(1,1)*Fthinv_matx(1,2) + dvD(1,2)*Fthinv_matx(2,2)
     &        + dvD(2,1)*Fthinv_matx(1,1) + dvD(2,2)*Fthinv_matx(2,1)
c
      Dtot(4) = 0.5 * Dtot(4)
c
c.....Spin.........
c
      Wt12  = dvD(1,1)*Fthinv_matx(1,2) + dvD(1,2)*Fthinv_matx(2,2)
     &      - dvD(2,1)*Fthinv_matx(1,1) - dvD(2,2)*Fthinv_matx(2,1)
      Wt12  = 0.5 * Wt12
c
c----------------------------------------------------------
...
c.......................................................
c
      Ed = el_E *(1.0 - el_V) /((1.0 + el_V)*(1.0 - 2.0 * el_V))
      Es = el_E * el_V /((1.0 + el_V)*(1.0 - 2 * el_V))
      G0 = el_E/(1.0 + el_V)
c	 
      C1111 = Ed
      C1122 = Es
...
c
c  *NOTE*:
c  There is very subtle point about initialing
...
c
     ...
     &              - Wt12*(sts_KH(1,1) - sts_KH(2,2)) 
         ...
         sts_dot(3) = sig_jrate(3)
c
         sts_CH(1,1) = sts_CH_n(1,1) + sts_dot(1)*dt/detFth
         sts_CH(1,2) = sts_CH_n(1,2) + sts_dot(4)*dt/detFth
         sts_CH(2,1) = sts_CH(1,2)
         sts_CH(2,2) = sts_CH_n(2,2) + sts_dot(2)*dt/detFth
         sts_CH(3,3) = sts_CH_n(3,3) + sts_dot(3)*dt/detFth
      endif
c
c-----------------------------------------------
