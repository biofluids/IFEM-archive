...
!-->
 !write structure coordinates
  do i=1, nn_solid
     write(i_file_unit, 101) i,solid_coor_curr(1:nsd_solid,i)
  enddo
101 format(i8,3e12.5)
 ...
 !...write structure part - connectivity
  ...
  elseif (nsd_solid == 2) then
     ...
     case (3) 
        write(i_file_unit,'(a7)') '  tria3'    ! element type
        ...
        enddo     
     case (4)
        write(i_file_unit,'(a7)') ' cubic4'    ! element type
        write(i_file_unit, '(i8)')  ne_solid   ! number of elements
        do i=1, ne_solid
  ...
  case (4)
     ...
     write(i_file_unit, *) 'cubic4'  ! element type
     write(i_file_unit, '(i8)')  ne   ! number of elements
     do i=1, ne
        write(i_file_unit,'(5i8)') i, (ien(j,i)+nn_solid,j=1,nen) !element connectivity
     enddo
...
subroutine zfem_ensFluid(d,f_fluids,solid_force_FSI,solid_vel,solid_pave,solid_stress,solid_strain,klok)
  ...
  real(8),dimension(nsd_solid*2,nn_solid) :: solid_stress  !...solid stress (Voigt notation)
  real(8),dimension(nsd_solid*2,nn_solid) :: solid_strain  !...solid strain (Voigt notation)
  integer :: klok
  
  real(8) :: fluid_stress(nsd*2,nn),fluid_strain(nsd*2,nn)              !...fluid stress and strain (not used) empty matrix needed for Ensight input format
 ...
 !...Write velocity output in ens_movie.vel*
  ...
  write(ifileunit,110) (solid_vel(1:nsd_solid,in),in=1,nn_solid), &
                       (d(1:nsd,in),in=1,nn)
  close(ifileunit)
 ...
 !...Write Interaction force output in ens_movie.fsi*
  ...
  write(ifileunit,110) (solid_force_FSI(1:nsd_solid,in),in=1,nn_solid), &
                       (f_fluids(1:nsd,in),in=1,nn)
  close(ifileunit)
 ...
 !...Write pressure output in ens_movie.pre*
  ...
  write(ifileunit,110) (solid_pave(in),in=1,nn_solid),(d(nsd+1,in),in=1,nn)
  close(ifileunit)
 ...
 !...Write stress output in ens_movie.stress*
  ...
  write(ifileunit,110) (solid_stress(1:nsd_solid*2,in),in=1,nn_solid),  &
                       (fluid_stress(1:nsd*2,in),in=1,nn)
  close(ifileunit)
 ...
 !...Write strain output in ens_movie.strain*
  ...
  write(ifileunit,110) (solid_strain(1:nsd_solid*2,in),in=1,nn_solid),  &
                       (fluid_strain(1:nsd*2,in),in=1,nn)
  close(ifileunit)
