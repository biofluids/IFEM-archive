...
subroutine gmres_correction(A,A_index,B,w,nn_inter,length,nn_count,nn_inter_loc)

  use fluid_variables, only:inner,outer
  use mpi_variables
  implicit none
  include 'mpif.h'
  integer nn_inter,length
  real(8) A(nn_inter,length)!,A_index(nn_inter,length)
  integer A_index(nn_inter,length)
  real(8) B(nn_inter)
  integer nn_count(nn_inter)
  real(8) w(nn_inter)
...
!  call getnorm(r0,r0,nn_inter,rnorm0)
  ...
  rnorm=sqrt(rnorm0)
if(myid==0)write(*,*)'begin gmres correction'
!+++++++++++++start outer loop+++++++++++++++++
...
if(myid==0)write(*,*)avloc(1:nn_inter)
stop
	space1(:)=0.0
	space2(:)=0.0
...
if(myid==0) write(*,*)'========time for gmres_correction=====',time
end subroutine gmres_correction
