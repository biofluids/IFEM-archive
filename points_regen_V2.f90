...
!  integer nn_inter                 !original # of interface points
  ...
  real(8) x_loc_can(nsd,500)       !local coordinats of candidate points per element           
  real(8) x_glo_can(nsd,500)       !global coordinats of candidat points per element
...
!      x_inter_regen_local(:,:)=0.0
      do while((nn_local.le.nn_cr).and.(nn_sub.le.19)) ! begin regeneration loop
...
!           write(*,*)'Ip_ini_can=',Ip_temp1
	   if( (signf*(Ip_temp1-Ic_inter) .le. 0.2*Ic_inter).and.(signf*(Ip_temp1-Ic_inter).ge.0.0)) then
!           if(((Ip_temp1-Ic_inter).le.(0.1*Ic_inter)).and.((Ic_inter-Ip_temp1).le.(0.1*Ic_inter))) then
		call point_projection(Ic_inter,temp,x_center,I_fluid_center,x_inter,corr_Ip,hs,Ip_temp1,err)
!		write(*,*)'err_regen=',err
